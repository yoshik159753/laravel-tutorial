...
[source,bash]
----
exec docker-compose up nginx mysql phpmyadmin // <1>
exec docker-compose exec workspace php artisan test // <2>
exec docker-compose exec workspace npm run watch // <3>
----
<1> docker 起動
<2> テスト実行
<3> assets の監視起動
